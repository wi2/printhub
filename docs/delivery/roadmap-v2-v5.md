...
1. **Canonical JSON as the intermediate format** — every resolved profile exists as a typed, versioned JSON document before serialization (ARCH-1, ARCH-2 from backlog; required by ADR-004)
2. **Profile versioning** — each time a combination's parameters change, a new `ProfileVersion` is created; old versions are preserved and queryable
3. **Feedback linkage** — every feedback submission references the `ProfileVersion` that was active when the profile was downloaded
4. **Profile generation history** — the system records which `ProfileVersion` was served to each generation request
5. **Profile evolution view** — profile authors can compare two `ProfileVersion` records side-by-side
...
- 100% of V2+ profiles have a canonical JSON record with a `schemaVersion` field
